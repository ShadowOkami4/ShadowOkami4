![Banner](https://github.com/ShadowOkami4/ShadowOkami4/assets/54201724/6dacd51c-0436-4bf3-a1bd-88b9cbfb64cf)
# 💻 Lukas | ShadowOkami

Welcome to my GitHub profile! I'm Lukas, known online as ShadowOkami (or simply Okami). I specialize in Python development, particularly in creating Discord bots with [Py-Cord](https://github.com/Pycord-Development/pycord) and crafting OBS Python scripts that connect via Discord webhooks to enhance streaming experiences.

## 🔧 Technologies & Tools

- **Languages**: Python
- **Frameworks**: Py-Cord
- **Tools**: OBS, Discord Webhooks

## 📈 GitHub Stats

![Okami's GitHub stats](https://github-readme-stats.vercel.app/api?username=ShadowOkami&show_icons=true&theme=radical)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=ShadowOkami&layout=compact&theme=radical)

## 🚀 Projects

### 📦 Discord Bots
- **[(WIP) TEMPOkami](#)**: a TempChannel Bot 
- **[(WIP)Narrator](#)**: a Narrator Bot to help with D&D Campains or other Storytelling using command groups and a database

### 🎥 OBS Scripts
- **[StreamNotifier Aka OBScord ](#)**: Sends messages via Discord webhook when someone starts streaming his stream in OBS.


## 📫 How to Reach Me

- **Discord**: .shadowokami
- **Email**: service@shadowokami.com

## 🌱 What I'm Currently Learning

- Advanced Python techniques
- Integrating more services with Discord bots
- Enhancing OBS scripts for better streaming management

Thanks for visiting my profile! Feel free to check out my repositories and reach out if you have any questions or collaboration ideas.
